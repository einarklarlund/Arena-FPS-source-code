Shader "Custom/Toon Lit Mapped Ordered Dither Surface"{
    ...
    Properties{
        ...
        [Header(Bump Map Parameters)]    
        _NormalMap ("Normal map", 2D) = "gray" {}
        _NormalMapTint ("Normal map tint", Color) = (1, 1, 1, 1)
        _ExtrusionMap ("Extrustion map", 2D) = "white" {}
        _Amount("Extrusion Amount", Range(0,3)) = 0.5
    }
    ...
    SubShader {
        ...
        //normal and extrusion mapping properties
        sampler2D _NormalMap;
        float4 _NormalMapTint;
        ...
        {
            float d = tex2D(_ExtrusionMap, float4(v.texcoord.xy,0,0)).r * _Amount;
        ...
        void surf (Input i, inout ToonSurfaceOutput o) {
            ...
            //calculate color from dither value
            ...
            //normal mapping
            // o.Normal = UnpackNormal(tex2D(_NormalMap, i.uv_NormalMap) * _NormalMapTint);
        }
        ENDCG
    }
    FallBack "Standard"

...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    Selectable m_LastSelectable = null;
    ...
    {
        WeaponController activeWeapon = GetActiveWeapon();
        activeWeapon.delayBetweenShots *= 0.3f;
        activeWeapon.bulletSpreadAngle *= 2f;
        Debug.Log(activeWeapon.bulletSpreadAngle);
    }
    ...
    void UpdateWeaponSwitching()
    {
        ...
        // if(!m_WeaponSwitchPostionReset && m_TimeStartedWeaponSwitch > weaponSwitchDelay)
        // {
        //     Debug.Log("resetting wepaon position at time " + Time.time);
        //     m_WeaponMainLocalPosition = defaultWeaponPosition.localPosition;
        //     m_WeaponSwitchPostionReset = true;
        // }
    }
    ...
    public Collider RaycastForSelectableCollider()
    {
        ...
        if(Physics.Raycast(weaponCamera.transform.position, weaponCamera.transform.forward, out hit, Mathf.Infinity, layerMask))
        {
            return hit.collider;
        }
